...
export const AdminBlogTab: React.FC<AdminBlogTabProps> = ({
  ...
  const [formData, setFormData] = useState({
    ...
    category: 'ai' as BlogPost['category'],
  ...
  const categories = [
    ...
    { id: 'development', label: 'تطوير البرمجيات' },
    { id: 'design', label: 'التصميم وتجربة المستخدم' },
    { id: 'cybersecurity', label: 'الأمن السيبراني' },
    { id: 'business', label: 'ريادة الأعمال التقنية' },
  ];
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    const postPayload: BlogPost = {
      ...
      publishedAt: editingPost ? editingPost.publishedAt : 'الآن',
      likesCount: editingPost ? editingPost.likesCount : 24,
      viewsCount: editingPost ? editingPost.viewsCount : 150,
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/75 backdrop-blur-sm animate-in fade-in" dir="rtl">
          <div className="relative w-full max-w-2xl bg-white rounded-3xl border border-slate-200 shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <form onSubmit={handleFormSubmit} className="p-6 space-y-4 overflow-y-auto flex-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  >
                    ...
                    <option value="business">ريادة الأعمال التقنية</option>
                  </select>
                </div>
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-700">رابط صورة الغلاف *</label>
                <input
                  type="url"
                  required
                  value={formData.coverImage}
                  onChange={(e) => setFormData({ ...formData, coverImage: e.target.value })}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-900 focus:outline-none focus:border-purple-500 text-left"
                  dir="ltr"
                />
              </div>
