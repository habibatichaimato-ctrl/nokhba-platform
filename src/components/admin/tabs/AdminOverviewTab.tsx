...
import { Product, BlogPost, JobListing, AdminActivityLog, AdminTab, OrderDetails } from '../../../types';
...
interface AdminOverviewTabProps {
  ...
  jobs: JobListing[];
  orders: OrderDetails[];
  visitStats: { total: number; today: number };
...
export const AdminOverviewTab: React.FC<AdminOverviewTabProps> = ({
  ...
  // إجمالي المبيعات الحقيقي: مجموع كل الطلبات الفعلية المسجّلة في قاعدة البيانات
  const estimatedRevenue = orders.reduce((acc, o) => acc + Number(o.total || 0), 0);
  const ordersCount = orders.length;
  const avgCart = ordersCount > 0 ? Math.round(estimatedRevenue / ordersCount) : 0;

  // إحصائيات آخر ٨ أشهر محسوبة فعلياً من تواريخ الطلبات الحقيقية
  const monthNames = ['يناير', 'فبراير', 'مارس', 'أبريل', 'مايو', 'يونيو', 'يوليو', 'أغسطس', 'سبتمبر', 'أكتوبر', 'نوفمبر', 'ديسمبر'];
  const now = new Date();
  const monthlyStats = Array.from({ length: 8 }).map((_, idx) => {
    const refDate = new Date(now.getFullYear(), now.getMonth() - (7 - idx), 1);
    const monthOrders = orders.filter((o) => {
      const od = new Date(o.createdAt);
      return od.getFullYear() === refDate.getFullYear() && od.getMonth() === refDate.getMonth();
    });
    const revenue = monthOrders.reduce((acc, o) => acc + Number(o.total || 0), 0);
    return { month: monthNames[refDate.getMonth()], sales: monthOrders.length, revenue };
  });

  const maxRevenue = Math.max(1, ...monthlyStats.map((m) => m.revenue));

  // نسبة النمو الحقيقية: مقارنة الشهر الحالي بالشهر السابق
  const currentMonthRevenue = monthlyStats[monthlyStats.length - 1].revenue;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 sm:gap-5">
        ...
        >
          <div className="flex items-center justify-between mb-3">
            ...
            <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full flex items-center gap-0.5 border ${growthPercent >= 0 ? 'text-emerald-600 bg-emerald-50 border-emerald-200' : 'text-rose-600 bg-rose-50 border-rose-200'}`}>
              <ArrowUpRight className={`w-3 h-3 ${growthPercent < 0 ? 'rotate-90' : ''}`} /> {growthPercent >= 0 ? '+' : ''}{growthPercent}%
            </span>
          </div>
          ...
          <div className="text-xl sm:text-2xl font-black text-slate-900 font-['Alexandria'] mt-1">
            {estimatedRevenue.toLocaleString()} <span className="text-xs font-bold text-amber-600">د.م</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[11px] font-bold text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded-full border border-indigo-200">
              اليوم: {visitStats.today}
            </span>
          </div>
          <span className="text-xs font-medium text-slate-500">زيارات الموقع الإجمالية</span>
          <div className="text-xl sm:text-2xl font-black text-slate-900 font-['Alexandria'] mt-1">
            {visitStats.total} <span className="text-xs font-semibold text-slate-500">زيارة</span>
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-bold text-slate-900 font-['Alexandria']">
                  مؤشر الإيرادات وحركة المتجر لعام {now.getFullYear()}
                </h3>
              </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-bold px-2.5 py-1 rounded-lg bg-slate-100 text-slate-700 border border-slate-200">
                متوسط السلة: {avgCart.toLocaleString()} د.م
              </span>
            </div>
          </div>
                ...
                return (
                  <div key={idx} className="flex-1 flex flex-col items-center gap-2 group relative">
                    ...
                    <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute -top-12 bg-slate-900 text-white text-[10px] font-bold px-2.5 py-1.5 rounded-xl shadow-lg pointer-events-none whitespace-nowrap z-20">
                      <span>{item.revenue.toLocaleString()} د.م ({item.sales} طلب)</span>
                    </div>
