...
export const AdminSettingsTab: React.FC<AdminSettingsTabProps> = ({ settings, onSave, addToast }) => {
  ...
  const handleRealBackup = async () => {
    setIsBackingUp(true);
    try {
      const tables = ['products', 'blog_posts', 'jobs', 'services', 'orders', 'job_applications', 'service_quotes'];
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-slate-900 rounded-3xl p-6 text-white border border-slate-800 shadow-sm space-y-4">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">تصدير كل بيانات المنصة الفعلية من قاعدة البيانات (المنتجات، المقالات، الوظائف، الخدمات، الطلبات) بصيغة JSON</p>
            </div>
