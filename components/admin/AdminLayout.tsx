...
// تنسيق معلومات النشر الحقيقية لعرضها بشكل مقروء بالعربية
const getDeploymentLabel = (): string => {
  if (__BUILD_COMMIT__ === 'local') {
    return 'نسخة تطوير محلية';
  }
  const date = new Date(__BUILD_DATE__).toLocaleDateString('ar-MA', {
    year: 'numeric',
    month: 'long',
    day: 'numeric'
  });
  return `آخر نشر: ${date} · ${__BUILD_COMMIT__}`;
};
...
export const AdminLayout: React.FC<AdminLayoutProps> = ({
  ...
  const getTabTitle = (tab: AdminTab) => {
    switch (tab) {
      ...
      case 'visits': return 'تحليلات زوار المنصة';
          ...
          <div className="flex items-center gap-2.5 sm:gap-3">
                  ...
                  <div className="space-y-2 max-h-72 overflow-y-auto">
                    {notifications.length === 0 && (
                      <p className="text-center text-[11px] text-slate-400 py-6">
                        لا توجد تنبيهات حالياً — ستظهر هنا تلقائياً أي طلبات شراء أو توظيف أو استشارة جديدة.
                      </p>
                    )}
                    {notifications.map((n) => (
                      <div
            ...
            <div className="flex items-center gap-2 pl-2 border-r border-slate-200 mr-1">
              {currentAdmin.avatar ? (
                ...
                <div className="w-8 h-8 rounded-full bg-amber-100 border border-amber-500 flex items-center justify-center text-amber-700 text-xs font-bold">
                  {currentAdmin.name?.charAt(0) || '?'}
                </div>
              )}
