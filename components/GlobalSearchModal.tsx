...
import { Product, BlogPost, JobListing, NavSection } from '../types';
...
export const GlobalSearchModal: React.FC<GlobalSearchModalProps> = ({
  ...
  const searchResults = useMemo(() => {
    if (!query.trim() || query.trim().length < 2) {
      return { products: [], blogPosts: [], jobs: [] };
    }
      ...
      <div className="relative w-full max-w-2xl bg-white border border-slate-200 rounded-3xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center gap-3 p-4 sm:p-5 border-b border-slate-100 bg-slate-50">
          ...
          <input
            ...
            placeholder="ابحث عن منتج، مقال، أو وظيفة في Nexus..."
          ...
          {!query.trim() ? (
            <div className="text-center py-8 space-y-3">
              ...
              <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
                ...
                {['ذكاء اصطناعي', 'لوحة مفاتيح', 'مهندس برمجيات', 'Thunderbolt'].map((tag) => (
                  <button
                    key={tag}
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <span className="text-[11px] text-amber-700 font-bold font-['Alexandria']">{p.price} د.م</span>
                          </div>
                        </div>
